...
CREATE TABLE IF NOT EXISTS `inventory_batches` (
  ...
  `main_stock` INT NOT NULL DEFAULT 0,
  `drawer_stock` INT NOT NULL DEFAULT 0,
  `date_arrived` DATE DEFAULT NULL,
  `expired_on` DATE DEFAULT NULL,
  `restock_semester` VARCHAR(50) DEFAULT '1st Semester',
...
CREATE TABLE IF NOT EXISTS `inventory_logs` (
  ...
  `action_type` ENUM('restock', 'dispense', 'dispose', 'adjust', 'transfer', 'edit') NOT NULL,
  `quantity_changed` INT NOT NULL,
  `source_location` ENUM('main', 'drawer') NULL,
  `target_location` ENUM('main', 'drawer') NULL,
